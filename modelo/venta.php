<?php
    ...
    class ventas{
        ...
        public function reporte_ventas($id){
            $sql="SELECT id_venta,fecha,cliente,venta.cedula,total,CONCAT(usuario.nombre,' ',usuario.apellido) as vendedor FROM venta JOIN usuario on vendedor=id_usuario WHERE id_venta=:id_venta";
            $query=$this->acceso->prepare($sql);
